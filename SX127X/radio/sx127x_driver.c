...
void sx127xInit(tLoRaSettings *stting)
{
    ...
    while (0x6c != Read127xReg(0x06))   /* 0x06:FR载波频率最高有效位 */
    {
	...
	/* 
		带宽设置：
			0000-7.8kHz		0001-10.4kHz 	0010-15.6kHz 	0011-20.8kHz
			0100-31.25kHz	0101-41.7kHz	0110-62.5kHz	0111-125kHz
			1001-500kHz
			
		带宽设置注意：低频段（169MHZ）不支持信号带宽 0111 和 1001

		纠错编码率：（隐式报头模式下应在接收机上设置该位，以确认预期编码率）
			001-4/5
			010-4/6
			011-4/7
			100-4/8 */
    ...
    Write127xReg(REG_LR_MODEMCONFIG2, u8_SFList[stting->SpreadingFactor - 6] | RFLR_MODEMCONFIG2_TXCONTINUOUSMODE_OFF | RFLR_MODEMCONFIG2_RXPAYLOADCRC_ON | 0x03); //设置SF，CRC，超时时间

	Write127xReg(REG_LR_SYMBTIMEOUTLSB, 0xFF);  // 设置超时时间
    Write127xReg(REG_LR_MODEMCONFIG3, 0x0C);    // 设置低速率(包长超过16ms必须打开)，设置内部AGC环路LNA增益

    if (stting->Power > 17)
    {
        Write127xReg(REG_LR_PACONFIG, 0x80 + stting->Power - 5); //设置功率
        Write127xReg(0x4d, 0x87); 	//设置更大功率（使用PA_LF或PA_HF时，高功率设置必须关闭）
    }
    else
    {
        Write127xReg(REG_LR_PACONFIG, 0x80 + stting->Power - 2); //设置功率
        Write127xReg(0x4d, 0x84); 	//关闭更大功率（使用PA_LF或PA_HF时，高功率设置必须关闭）
    }
...
{
    SX127X_ResetPinControl(0);      // 复位引脚拉低
    Soft_delay_ms(10);
    SX127X_ResetPinControl(1);      // 复位引脚拉高
    Soft_delay_ms(10);
    loraStatus = RFLR_STATE_IDLE;   // RF设置空闲态
}
...
void SX127xSetFrf(uint32_t fr)
{
    ...
    Write127xReg(REG_LR_FRFMSB, frfBuf[2]);	// RF载波频率最高有效位
    Write127xReg(REG_LR_FRFMID, frfBuf[1]);	// RF载波频率中间有效位
    Write127xReg(REG_LR_FRFLSB, frfBuf[0]);	// RF载波频率最低有效位
}
...
tRFProcessReturnCodes SX127xProcess(void)
{
    ...
    case RFLR_STATE_RX_INIT:
        SX127xSetOpMode(LORA_OPMODE_STANDBY);		// 设置成待机模式
        
        Write127xReg(REG_LR_FIFORXBASEADDR, 0);  	// 将接收buf的基地址指向0x00，此时整个fifo都可以用来接收数据
        Write127xReg(REG_LR_FIFOADDRPTR, 0);  		// 将fifi读写指针执行0x00，此时向寄存器0x00读写数据指针会自增的从fifo中读取数据
        Write127xReg(REG_LR_SYMBTIMEOUTLSB, 0xFF);  // 接收超时配置
